...
import { CheckCircle2, Lock as LockIcon, AlertTriangle, ChevronRight, ChevronLeft } from 'lucide-react';
...
export default function DiagnosisFlow() {
  ...
  const currentBlock = DIAGNOSIS_STRUCTURE.find(b => b.id === currentBlockIndex);
  ...
  const saveProgress = async (answers: Record<string, any>, nextBlock?: number) => {
    ...
    const totalBlocks = 8;
    const completedBlocks = Object.keys(updatedAnswers).filter(k => k.startsWith('block_') && k !== 'block_0' && k !== 'block_9').length;
    const progress = Math.min(Math.round((completedBlocks / totalBlocks) * 100), 100);
  ...
  const handleInputChange = (id: string, value: any, multiple?: boolean) => {
    ...
    if (multiple) {
      const currentValues = Array.isArray(localAnswers[id]) ? localAnswers[id] : [];
      let newValues;
  ...
  const finalizeDiagnosis = async () => {
    ...
    try {
      ...
      let aiAnalysisData;
      try {
        aiAnalysisData = await generateDiagnosisReasoning(scores, affinities, textualAnswers);
      } catch (genErr) {
        console.error("Error generating reasoning. Fallback to basic.", genErr);
        aiAnalysisData = {
          perfil_predominante: affinities[0]?.name || "No identificado",
          perfil_secundario: (affinities[1]?.affinity || 0) >= 45 ? affinities[1].name : "Ninguno",
          afinidad_predominante: affinities[0]?.affinity || 0,
          razonamiento: {
            diagnostico_central: "Error al generar análisis cualitativo profundo. Revisa la configuración de API.",
            evidencias: [],
            contradicciones: [],
          ...
          recomendaciones: ["Resolver la conexión con tu avatar", "Clarificar tu oferta", "Comenzar a documentar"],
          devolucion_personalizada: "Basado en tu perfil cuantitativo, estás priorizando áreas técnicas. Necesitamos indagar cualitativamente en sesión."
        };
      }
      ...
      setAnalyzing(false);
      alert("Hubo un error al procesar el análisis. Por favor intenta de nuevo.");
    }
  };
        ...
        <div className="relative z-10 flex flex-col items-center max-w-sm w-full">
          <div className="relative w-24 h-24 mb-12">
            ...
            <div className="absolute inset-4 rounded-full bg-slate-900 border border-slate-800 flex items-center justify-center shadow-[0_0_20px_rgba(245,158,11,0.2)]">
              <span className="text-amber-500 text-xs font-bold font-mono">{Math.min(analysisStep * 25 + 10, 99)}%</span>
            </div>
          </div>
  ...
  const totalBlocks = 8;
  const completedBlocksCount = Object.keys(diagnosis?.answers || {}).filter(k => k.startsWith('block_') && k !== 'block_0' && k !== 'block_9').length;
  const currentProgress = Math.min(Math.round((completedBlocksCount / totalBlocks) * 100), 100);
      ...
      <aside className="w-72 border-r border-slate-800 bg-slate-900/30 hidden lg:flex flex-col">
        <div className="p-6 flex-1 overflow-y-auto">
          <ul className="space-y-1">
            ...
            >
              <div className={`w-6 h-6 rounded-full border flex items-center justify-center text-[10px] font-bold ${currentBlockIndex === 0 ? 'border-amber-500 text-amber-500' : 'border-slate-600'}`}>00</div>
              <span className={`text-sm font-semibold ${currentBlockIndex === 0 ? 'text-white' : ''}`}>Bienvenida</span>
            </li>
                ...
                <li 
                  ...
                  className={`flex items-center gap-3 p-3 rounded-r-md group transition-all cursor-pointer ${
                    isActive 
                      ...
                      : isBlockCompleted ? 'opacity-100' : 'opacity-40 hover:opacity-100'
                  }`}
                >
                  <div className={`relative w-6 h-6 rounded-full border flex items-center justify-center text-[10px] font-bold transition-colors ${
                    isActive ? 'border-amber-500 text-amber-500' : isBlockCompleted ? 'border-amber-500 text-amber-500' : 'border-slate-600'
                  }`}>
                    {isBlockCompleted && !isActive ? <CheckCircle2 size={10} /> : block.id.toString().padStart(2, '0')}
                  ...
                  <span className={`text-sm font-medium ${isActive ? 'text-white font-semibold' : ''}`}>{block.name}</span>
                </li>
              );
            })}
          </ul>
        ...
        <div className="p-6 border-t border-slate-800 text-[10px] text-slate-500 tracking-wider leading-relaxed">
          SISTEMA DE DIAGNÓSTICO ESTRATÉGICO v2.4<br/>SYSTEAM LATAM © 2026
        </div>
      </aside>
      ...
      <div className="flex-1 bg-slate-950 overflow-hidden flex flex-col pt-0 relative">
        ...
        <div className="lg:hidden bg-slate-900 border-b border-slate-800 p-4">
          <div className="flex items-center justify-between mb-2">
            <span className="text-xs text-slate-400 font-black uppercase tracking-widest">Progreso</span>
            <span className="text-xs text-amber-500 font-black">{currentProgress}%</span>
          </div>
          ...
          <div className="max-w-3xl mx-auto py-8 md:py-12">
            ...
            {isCompleted && (
              <div className="mb-10 bg-amber-500/10 border border-amber-500/20 p-4 rounded-sm flex items-center gap-4">
                ...
                <p className="text-amber-200 text-sm font-medium">Diagnóstico completado. Las respuestas ya no son editables.</p>
              </div>
            )}
            ...
            {currentBlock.type === 'welcome' ? (
              ...
              >
                <div className="bg-slate-900 border border-slate-800 p-8 md:p-12 rounded-xl shadow-2xl relative mb-12">
                  <div className="absolute -top-3 left-8 px-4 py-1 bg-amber-500 text-slate-950 text-[10px] font-black uppercase tracking-tighter rounded-sm">Mensaje de Bienvenida</div>
                  <p className="text-xl md:text-2xl leading-relaxed text-slate-300 italic font-serif">
                    "Antes de comenzar la Incubadora, quiero invitarte a hacer este diagnóstico con total honestidad. <span className="text-amber-400">Este no es un examen. Es un mapa.</span> La mayoría de los expertos cree que necesita más contenido o más estrategia. Pero muchas veces el bloqueo está en la identidad desde la que estás construyendo... responde desde la verdad, no desde lo que crees que 'debería' ser."
                  </p>
                </div>
                <div className="prose prose-invert max-w-none text-slate-400 text-lg space-y-6">
                  <p>Cuanto más honesta sea tu respuesta, más potente será tu proceso.</p>
                </div>
              </motion.div>
            ) : (
              <div className="space-y-12">
                <div className="flex flex-col md:flex-row md:items-end gap-3 md:gap-4">
                  <h2 className="text-3xl md:text-4xl font-light text-white">{currentBlock.title}</h2>
                  {currentBlockIndex !== 9 && (
                    <span className="text-amber-500 font-mono mb-1 tracking-widest text-sm uppercase">BLOQUE_{currentBlockIndex.toString().padStart(2, '0')}</span>
                  )}
                </div>

                <form className="space-y-12" onSubmit={(e) => e.preventDefault()}>
                  {visibleQuestions.map((q) => (
                    <div key={q.id} className={`space-y-4 ${isCompleted ? 'opacity-80' : ''}`}>
                      <label className="label-geometric">{q.label} {q.multiple && !isCompleted && <span className="text-[10px] text-amber-500 lowercase ml-2 font-normal tracking-normal">(Multi-selección)</span>}</label>
                      
                      {q.type === 'text' && (
                        <DictationTextarea 
                          className="input-geometric min-h-[120px] focus:border-amber-500/50"
                          placeholder={isCompleted ? "Diagnóstico completado." : "Escribe tu respuesta..."}
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                          {q.options?.map((opt) => {
                            const isSelected = q.multiple 
                              ? Array.isArray(localAnswers[q.id]) && localAnswers[q.id].includes(opt)
                            ...
                            return (
                              <button
                                ...
                                type="button"
                                disabled={isCompleted}
                                onClick={() => handleInputChange(q.id, opt, q.multiple)}
                              ...
                              >
                                <span className={`mr-2 transition-colors inline-block ${isSelected ? 'text-amber-500' : 'group-hover:text-amber-500'}`}>
                      ...
                      {q.type === 'scale' && (
                        <div className="flex items-center gap-4">
                          <span className="text-[10px] text-slate-500 font-bold uppercase tracking-widest hidden sm:inline-block">Nada</span>
                          <div className={`flex-grow flex justify-between bg-slate-900 p-1 rounded-sm border border-slate-800 overflow-x-auto gap-1 ${isCompleted ? 'cursor-default' : ''}`}>
                            {[1, 2, 3, 4, 5].map((val) => (
                              <button
                                key={val}
                                type="button"
                          ...
                          <span className="text-[10px] text-slate-500 font-bold uppercase tracking-widest hidden sm:inline-block">Total</span>
                        </div>
                      )}
                    </div>
                  ))}
        ...
        <div className="mt-auto flex flex-col md:flex-row items-center justify-between p-6 md:px-12 border-t border-slate-900 bg-slate-950">
          <div className="flex items-center gap-2 mb-4 md:mb-0">
            {!isCompleted ? (
              <>
                <div className={`w-2 h-2 rounded-full ${saving ? 'bg-amber-500 animate-pulse shadow-[0_0_8px_rgba(245,158,11,0.6)]' : 'bg-slate-600'}`}></div>
          ...
          <div className="flex gap-4 w-full md:w-auto">
            ...
            >
              {currentBlockIndex === 9 ? (isCompleted ? 'Ir a resultados' : 'Finalizar y Analizar') : 'Siguiente'}
            </button>
          </div>
        </div>
      </div>
            ...
            >
              <div className="p-8">
                ...
                <p className="text-slate-300 text-sm text-center font-medium leading-relaxed mb-10">
                  Una vez enviado, <strong className="text-amber-400 font-bold text-base">no podrás modificar tus respuestas.</strong> Recibirás tu perfil personalizado y tus recomendaciones estratégicas inmediatamente.
                </p>
